...
public class ParserTest
{
   ...
   /* @Test
    ...
    {
        RGBColor actual = myParser.makeExpression("1", myParser).evaluate();
        assertTrue(WHITE.equals(actual));
        actual = myParser.makeExpression("-1", myParser).evaluate();
        assertTrue(BLACK.equals(actual));
        actual = myParser.makeExpression("0.5", myParser).evaluate();
        assertTrue(GRAY.equals(actual));
        actual = myParser.makeExpression(".5", myParser).evaluate();
        assertTrue(GRAY.equals(actual));
        try
        {
